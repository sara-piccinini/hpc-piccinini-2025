...
double* read_matrix_from_csv(const char* filename, int N) {
    ...
    while (fgets(line, sizeof(line), file) && row < N) {
        ...
        int col = 0;
        while (token && col < N) {
            matrix[row * N + col] = strtod(token, NULL);
            token = strtok(NULL, ",");
            ++col;
        }
        ++row;
...
// Write matrix to CSV
void write_matrix_to_csv(const char* filename, const double* matrix, int N) {
...
int main(int argc, char** argv) {
    ...
    int P = (int)sqrt(size);
    if (P * P != size || N % P != 0) {
        if (rank == 0) fprintf(stderr, "Size %d not compatible with process grid %d\n", N, size);
        MPI_Finalize(); return 1;
    }
    ...
    if (rank == 0) {
        ...
        sprintf(pathA, "input_matrices/A_%d.csv", N);
        sprintf(pathB, "input_matrices/B_%d.csv", N);
        A_full = read_matrix_from_csv(pathA, N);
        B_full = read_matrix_from_csv(pathB, N);
    }
    ...
    for (int i = 0; i < my_row; ++i)
        MPI_Sendrecv_replace(A, block_size * block_size, MPI_DOUBLE, dst_A, 100, src_A, 100, cart_comm, MPI_STATUS_IGNORE);
    for (int i = 0; i < my_col; ++i)
        MPI_Sendrecv_replace(B, block_size * block_size, MPI_DOUBLE, dst_B, 200, src_B, 200, cart_comm, MPI_STATUS_IGNORE);
    ...
    if (rank == 0) {
        ...
        FILE* tlog = fopen("timings/results.csv", "a");
        if (tlog) {
            fprintf(tlog, "%d,%d,%.6f\n", N, size, end - start);
            fclose(tlog);
        }
    ...
    if (rank == 0) {
        char outpath[256];
        snprintf(outpath, sizeof(outpath), "sara/test/C_out_%d_np%d.csv", N, size);
        write_matrix_to_csv(outpath, C_full, N);
        printf("Matrix multiplication complete. Result saved to %s\n", outpath);
        free_matrix(C_full);
        free_matrix(A_full);
        free_matrix(B_full);
    }
